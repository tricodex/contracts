...
import { abi as ABI } from "./abis/CustomizableAgent.json";
...
async function main() {
    ...
    const contractAddress = process.env.CUSTOMIZABLE_AGENT_ADDRESS;
    if (!contractAddress) throw Error("Missing CUSTOMIZABLE_AGENT_ADDRESS in .env");

    const provider = new ethers.JsonRpcProvider(rpcUrl);
    const wallet = new Wallet(privateKey, provider);
    const contract = new Contract(contractAddress, ABI, wallet);

    console.log("Welcome to the CustomizableAgent CLI!");
    console.log("----------------------------------------");
    ...
    while (true) {
        ...
        const choice = await getUserInput("Enter your choice (1 or 2): ");
...
async function startNewAgentRun(contract: Contract) {
    ...
    const transactionResponse = await contract.startAgentRun(query, interactionType, maxIterations);
