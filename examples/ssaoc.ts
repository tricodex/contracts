...
import { abi as ABI } from "./abis/SSAOC.json";
...
async function main() {
    console.log("Initializing SSAOC script...");

    const rpcUrl = process.env.RPC_URL;
    const privateKey = process.env.PRIVATE_KEY;
    const contractAddress = process.env.SSAOC_CONTRACT_ADDRESS;
    ...
    contract.on("Debug", (message: string, runId: number) => {
        console.log(`Debug Event - Message: "${message}", Run ID: ${runId}`);
    });

    // Listen for "AgentRunCreated" events
    contract.on("AgentRunCreated", (owner: string, runId: number) => {
        console.log(`AgentRunCreated Event - Owner: ${owner}, Run ID: ${runId}`);
    });
    ...
    if ((await getUserInput("Change system prompt? (y/n): ")).toLowerCase() === 'y') {
        ...
        console.log("Waiting for prompt update to be processed...");
    ...
    while (true) {
        ...
        if (Date.now() - startTime > maxWaitTime) {
            console.log("Maximum wait time exceeded. Exiting.");
            break;
        }
