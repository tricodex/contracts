...
interface IOracle {
    ...
    // Regular LLM call function
    function createLlmCall(
        uint promptId
    ) external returns (uint);

    // Normal OpenAI LLM Call (unstructured)
    function createOpenAiLlmCall(
        uint promptId,
        OpenAiRequest memory request
    ) external returns (uint);

    // Structured OpenAI LLM Call with parsed response
    function createStructuredOpenAiLlmCall(
        uint promptId,
        OpenAiRequest memory request,
        string memory responseFormat
    ) external returns (uint);

    // Function call for specific tools or functions
